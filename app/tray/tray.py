"""System-tray launcher — owns the facilitation-suite webapp lifecycle behind a tray icon.
...
reclaim-then-start owned by the shared ``tray_lifecycle.ps1``). Menu:
    ...
    Open facilitation-suite      — open the app (the tailnet HTTPS URL when the served
...
TRAY_ICON = PROJECT_ROOT / "assets" / "tray" / "facilitation-suite.ico"
...
MUTEX_NAME = r"Global\facilitation-suite-tray"
...
def _notify(title: str, message: str) -> None:
    ...
    try:
        ...
        Notification(app_id="facilitation-suite", title=title, msg=message).show()
...
class TrayApp:
    ...
    def _start(self) -> None:
        try:
            ...
            _notify("facilitation-suite ready", self.manager.public_url)
            ...
            _notify("facilitation-suite start failed", str(exc))
    ...
    def _on_wedge(self, count: int) -> None:
        if self.manager.is_port_in_use():
            ...
            logger.error("❌ %s", msg)
            _notify("facilitation-suite webapp wedged", "Listening but not answering — use tray.bat --restart")
            return
        ...
        try:
            ...
            _notify("facilitation-suite webapp respawned", self.manager.base_url)
        except Exception as exc:  # noqa: BLE001
            self.wd_log(f"webapp respawn failed: {exc}")
            logger.error("❌ webapp respawn failed: %s", exc)
            _notify("facilitation-suite respawn failed", str(exc))
    ...
    def _on_recover(self) -> None:
        ...
        _notify("facilitation-suite webapp recovered", self.manager.base_url)
    ...
    def restart_webapp(self, icon, item) -> None:  # noqa: ARG002
        def _do_restart() -> None:
            try:
                _notify("facilitation-suite", "Restarting webapp…")
                self.manager.restart(wait=True)
                self.wd_log("webapp restarted from tray menu")
                _notify("facilitation-suite webapp restarted", self.manager.public_url)
            except Exception as exc:  # noqa: BLE001
    ...
    def run(self) -> int:
        ...
        icon = pystray.Icon("facilitation-suite", icon=_build_icon(), title="facilitation-suite", menu=menu)
        icon.run()
...
def run_tray(config: AppConfig) -> int:
    ...
    if not instance.acquired:
        logger.info("ℹ️ Another facilitation-suite tray is already running; exiting.")
        return 0

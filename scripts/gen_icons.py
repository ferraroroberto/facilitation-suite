...
master art is the vendored Lucide ``presentation`` glyph in ``brand/``
...
icon-512.png,icon-512-maskable.png}``, ``assets/tray/facilitation-suite.ico`` and
``assets/stream-deck/facilitation-suite-144.png``. All committed — the runtime never
imports ``resvg-py``.
...
MASTER_SVG = PROJECT_ROOT / "brand" / "presentation.svg"
...
def main() -> int:
    ...
    render_set(
        ...
        project_slug="facilitation-suite",
    )
